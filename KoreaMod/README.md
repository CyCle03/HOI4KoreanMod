...
3. **CHO 태그는 게임 규칙 뒤에 있습니다.** 기본값은 `LIBERATION` — 1936년 지도를 전혀
   건드리지 않으므로 일본 AI에 영향이 없습니다. `GOVERNORATE` 를 고르면 설계서의 본래
   의도대로 두 주가 CHO로 넘어가고 일본의 통합 괴뢰국이 됩니다. 설계서 자신의 작업
   순서(1단계 해방국 → 5단계 CHO)와 위험 목록을 그대로 따른 것입니다.
